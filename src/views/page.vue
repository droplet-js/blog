<template>
  <Container :open="loading">
    <mu-container>
      <mu-paper :key="index" :z-depth="1" class="item-paper" v-for="(page, index) in pages">
        <mu-flex align-items="center" class="item-paper-title" justify-content="between">
          ...
          <mu-button :ripple="true" @click="gotoDetail(page)" class="item-paper-button" color="secondary">阅读</mu-button>
        </mu-flex>
        <p class="item-paper-sub-title">{{page.title}}</p>
      ...
      <mu-flex class="item-pagination" justify-content="center">
        <mu-pagination :current.sync="pagination.current" :page-count="pageCount" :page-size="pagination.pageSize" :total="pagination.total" @change="loadMore"></mu-pagination>
      </mu-flex>
    </mu-container>
  </Container>
</template>
